...
let environment = config.environment; //You can change environment by updating value of "environment" key of config.json

fixture `Swag Labs Flow Automation`
    .page `${config[environment].baseUrl}`;

test('Check total price of cart', async t => {
    let case_params = test_params.dataset_1;
    let item_count = case_params.item_count_to_add;
    await LoginPage.login(`${config[environment].valid_username}`, `${config[environment].valid_password}`);
    
    await InventoryPage.ensure_page(environment, "inventory_address");
    ...
    await CartPage.ensure_page(environment, "cart_address");
    await CartPage.verify_item_count(item_count);
    await CartPage.go_to_checkoutOne();

    await CheckoutOnePage.ensure_page(environment, "checkout_one_address");
    ...
    await CheckoutTwoPage.ensure_page(environment, "checkout_two_address");
